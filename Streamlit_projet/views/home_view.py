...
class HomeView:
    ...
    def render_hero():
        ...
        # Inject download link beside Browse files via components.html (real iframe)
        if csv_path.exists():
            components.html("""
<script>
(function(){
    ...
    function inject(){
        var btn = par.document.querySelector('[data-testid="stFileUploaderDropzone"] button');
        if(!btn){ setTimeout(inject, 150); return; }
        if(par.document.getElementById('dl-csv-link')) return;
        var a = par.document.createElement('a');
        a.id = 'dl-csv-link';
        ...
        a.style.cssText = 'font-family:\'Space Mono\',monospace;font-size:.85rem;color:#00e5c0;text-decoration:none;white-space:nowrap;margin-right:.9rem;letter-spacing:.03em;cursor:pointer;';
