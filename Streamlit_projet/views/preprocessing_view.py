...
class PreprocessingView:
    ...
    def render_correlation(df: pd.DataFrame,
        ...
        # Per requirements: exclude State/country encoded columns from correlation
        exc = (enc_cols or [])
        corr_cols = [c for c in num_cols if c != TARGET and c not in exc
                     and not any(c.startswith(e + '_') or c.startswith(e + '_bin')
                                 for e in (enc_cols or []))]
